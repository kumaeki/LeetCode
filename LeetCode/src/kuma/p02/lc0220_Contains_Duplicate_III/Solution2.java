...
public class Solution2 {
    ...
    public boolean containsNearbyAlmostDuplicate(int[] nums, int k, int t) {
        ...
        for (int i = 0; i < nums.length; i++) {
            ...
            if ((pre != null && num <= pre + t) || (next != null && next - t <= num))
                return true;
            tree.add(num);
            if (i >= k)
    ...
    @Test
    public void simple1() {
        assertEquals(true, new Solution2().containsNearbyAlmostDuplicate(new int[] { 3, 6, 0, 2 }, 2, 2));
    }

    @Test
    public void simple2() {
        assertEquals(false, new Solution2().containsNearbyAlmostDuplicate(new int[] { -1, Integer.MAX_VALUE }, 1,
                Integer.MAX_VALUE));
    }
    ...
    public void simple3() {
        assertEquals(false,
                new Solution2().containsNearbyAlmostDuplicate(new int[] { 0, 10, 22, 15, 0, 5, 22, 12, 1, 5 }, 3, 3));
    }
}
